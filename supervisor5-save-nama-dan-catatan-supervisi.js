...
import { getDatabase, get, update, ref, child } from "https://www.gstatic.com/firebasejs/10.7.0/firebase-database.js"
    ...
    if (user) {
        ...
        request.onsuccess = function () {
            ...
            getRequest.onsuccess = function (event) {
                ...
                updateNameNote(data.name, data.note)
            }
        }
...
async function updateNameNote(name, note) {
    try {
        ...
        const url = 'teachers/' + makerId + '/questions/' + token
        const dataRef = child(dbRef, url)
        update(dataRef, {
            "namaSupervisi": name,
            "catatanSupervisi": note
        }).then(() => {
            localStorage.setItem("pesan", "Data saved successfully|betul")
            location.href = location.origin + "/supervisor3-supervisi-page"
        }).catch((err) => {
...
setTimeout(() => {
    localStorage.setItem("pesan", "Slow network, please try again|gagal")
    location.href = location.origin + "/supervisor3-supervisi-page"
}, 10000)
